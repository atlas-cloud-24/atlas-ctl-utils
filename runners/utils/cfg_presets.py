"""§Phase 62: preset composition for platform cfg.
...
def declared_params(preset_dir: Path) -> list[str]:
    ...
    names: list[str] = []
    for entry in raw:
        # §Phase 62 D2: a bare list of names. A mapping here is the rejected
...
def _bind_params(text: str, bindings: dict, *, label: str, params: list[str]) -> str:
    ...
    §Phase 62 O2: a bound value may itself contain `${y}`. It is substituted as
...
def _project_keys(doc: dict, selection, *, label: str) -> dict:
    """§Phase 62 O1: `import` names cfg KEYS, never files.
