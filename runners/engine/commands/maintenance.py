...
class CtlStateMaintenance:
    """The explicit operations an operator runs over ctl-state records."""
    ...
    def history_prune(
        ...
        *,
        ...
        if args.prune_kind:
            selected_ids = {
                run_id for run_id in selected_ids if run_kinds.get(run_id) == args.prune_kind
            }
        unknown = sorted(selected_ids - set(run_keys))
        if unknown:
            raise RuntimeError(
                "❌ selected prune run ids are not present in this namespace: " + ", ".join(unknown)
            )
        ...
        if protected:
            raise RuntimeError(
                "❌ current committed revisions cannot be pruned: " + ", ".join(protected)
            )
        ...
        for snapshot_path in namespace_root.rglob(state_run_store.RUN_METADATA_FILENAME):
            ...
            workflow_run = str(snapshot.get("run_id") or snapshot_path.parent.name)
        ...
        deletion_keys = sorted(key for run_id in candidates for key in run_keys.get(run_id, []))
        ...
        manifest_path = (
            namespace_root / "_maintenance" / "history-prune" / maintenance_id / "manifest.yaml"
        )
    ...
    def run(
        ...
        *,
        ...
        context = execution_run_context.build_execution_context(
            ...
            force_skip_full_cfg_validation_gate=(args.force_skip_full_cfg_validation_gate),
            execution_runtime_mode=args.execution_runtime_mode,
    ...
    @staticmethod
    def inspect_selected_graph_backend(
...
def _maintenance_context(ctl_cfg_root: Path, args: argparse.Namespace, **overrides) -> dict:
    """The execution context every ctl-state operation opens with.

    Stated once: three operations derived it identically, so a field added to one
    was a field silently missing from the others.
    """
    ...
    return execution_run_context.build_execution_context(ctl_cfg_root, **fields)
...
def run_maintenance(
    ...
    if not target_cfg_dir.is_dir():
        raise RuntimeError(
            f"❌ target_run input cfg dir not found for target_run '{target_run_id}': {target_cfg_dir}"
        )
