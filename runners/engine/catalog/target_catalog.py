...
class TargetCatalog:
    """Declared targets, and the cfg an action composes from them."""
    ...
    def action_cfg(
        ...
        all_targets = TargetCatalog.load(ctl_cfg_root)
        targets = {}
    ...
    @staticmethod
    def consent_opt_in_fields(ctl_cfg_root=None) -> set[str]:
    ...
    @staticmethod
    def validate_distinct_signatures(definitions: dict) -> None:
...
class TargetExecutionIdentity:
    """The identity a target runs as: who, in which account, with which roles."""

    @staticmethod
    def validate_all(entries: object, *, label: str) -> dict:
        ...
        for provider, execution in entries.items():
            ...
            resolved[provider] = TargetExecutionIdentity.validate(
                execution, label=f"{label} [{provider}]"
            )
        return resolved

    @staticmethod
    def validate(execution: object, *, label: str) -> dict:
    ...
    @staticmethod
    def describe(execution: object) -> str | None:
        ...
        def _entry(provider: str | None, entry: dict) -> str:
            parts = [
                str(provider or entry.get("provider") or "?"),
        ...
        if all(isinstance(value, dict) for value in execution.values()):
            return ", ".join(
                _entry(provider, entry) for provider, entry in sorted(execution.items())
            )
        return _entry(None, execution)


class ActiveTargetRuns:
    """The target runs one invocation selected, and the facts they carry."""
    ...
    def build(
        ...
        def normalize_cfg_root(raw_value, *, target_key: str) -> str:
            ...
            if not isinstance(value, str) or not value.strip():
                raise RuntimeError(
                    f"Target run target {target_key!r} cfg_root must be a non-empty string"
                )
            value = value.strip()
            if "\\" in value:
                raise RuntimeError(
                    f"Target run target {target_key!r} cfg_root must use forward slashes: {value}"
                )
            if not value.startswith("/"):
                raise RuntimeError(
                    f"Target run target {target_key!r} cfg_root must start with /: {value}"
                )
            parts = [part for part in value.split("/") if part]
            if any(part in (".", "..") for part in parts):
                raise RuntimeError(
                    f"Target run target {target_key!r} cfg_root must not contain . or ..: {value}"
                )
        ...
        for st in workflow_cfg.get("target_runs", []):
            ...
            if not isinstance(target_source, str) or not target_source:
                raise RuntimeError(
                    f"Target run target {target_key!r} must define non-empty 'source'"
                )
            ...
            if require_branch_or_commit and not branch and not commit:
                raise RuntimeError(
                    f"Target run {target_run_id!r} source {target_source!r} has neither branch nor commit configured"
                )
            ...
            for behavior_field in (
                ...
                *sorted(TargetCatalog.consent_opt_in_fields()),
            ):
                if target_cfg.get(behavior_field) is not None:
                    active_target_run[behavior_field] = target_cfg[behavior_field]

            target_execution_identity = target_override.get(
                "execution_identities"
            ) or target_cfg.get("execution_identities")
            if target_execution_identity is not None:
                active_target_run["execution_identities"] = TargetExecutionIdentity.validate_all(
    ...
    @staticmethod
    def target_keys(active_target_runs: dict) -> list[str]:
        keys: list[str] = []
        seen: set[str] = set()
    ...
    @staticmethod
    def attach_definition_facts(active_target_runs: dict) -> None:
        for target_run in active_target_runs.values():
            definition = ActiveTargetRuns.definition_document(target_run)
            target_run["target_definition"] = definition
            target_run["target_definition_sha256"] = kernel_paths.canonical_sha256(definition)

    @staticmethod
    def definition_document(target_run: dict) -> dict:
        ...
        definition = {
            key: target_run[key] for key in stable_keys if target_run.get(key) is not None
        }
        modules = {}
...
class TargetEntries:
    """Target references as written in cfg, normalized to one shape."""
        ...
        for value in values:
            if isinstance(value, dict):
                ...
                if action is not None and (
                    not isinstance(action, str) or action not in run_actions.WORKFLOW_ACTIONS
                ):
                    raise RuntimeError(
    ...
    def validate_selectors(
        ...
        for entry in workflow_cfg.get("target_runs", []):
            ...
            if not run_selectors.selector_matches(
                selectors, execution_context, label=f"target {target_name}"
            ):
                raise RuntimeError(
...
class TargetActionPolicy:
    ...
    def actions(self) -> list[str]:
        ...
        if "operations" in self.entry:
            raise RuntimeError(f"❌ {self.label} target action policy does not accept 'operations'")
        if (
