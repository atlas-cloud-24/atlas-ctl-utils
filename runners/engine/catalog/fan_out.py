"""What a fan-out declares, and the child runs it expands into."""
...
class FanOutCatalog:
    """Reads fan-out cfg and resolves it into the concrete children to run."""

    @staticmethod
    def expand(ctl_cfg_root: Path, fan_out_key: str, execution_context: dict[str, object]) -> dict:
        """Expand a fan-out into concrete child runs: cfg only, no execution and no state.

        A param-set member whose selectors do not match the frozen execution
        context is DROPPED before children are built, so one fan-out serves every
        zone. Each child keeps its param-set and entry keys, so a report never
        conflates one declared workflow with its concrete expansions.
        """
    ...
    def validate_unique_materializations(
        ...
        """Reject a fan-out whose children would write the same state owner twice."""
    ...
    def wrap_preflight_child(
        ...
        *,
        ...
        """Fold a child's own params onto its node, under a param-set node when one expanded.

        Run-constant params (provider, landing_zone, …) live on the fan-out
        header, not here.
        """
