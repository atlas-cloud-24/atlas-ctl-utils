...
class AccountRegistryTests(unittest.TestCase):
    def _root(self, body: str):
        ...
        # selected-access checks run through the adapter, reached only via the
        # providers this root declares
...
class AccountSlugTests(unittest.TestCase):
    ...
    def test_missing_slug_is_rejected(self):
        ...
        with temporary, self.assertRaisesRegex(RuntimeError, r"ctl_plane\.slug is required"):
            aws.load_aws_account_slugs(
                root, execution_context={"execution_context.params.landing_zone": "live"}
            )
...
class HistoryPruneTests(unittest.TestCase):
    ...
    def _run(self, root, args, reader, maintainer=None):
        ...
        with (
            # Granted at the cfg read, not by stubbing an accessor: a permission
            # has one form now, and the profile is what says yes.
            mock.patch.object(
                run_policy,
                "ctl_profile_bool",
                side_effect=lambda root, profile, key: (
                    key == run_policy.Permissions.CTL_STATE_HISTORY_MAINTENANCE.key
                ),
            ),
