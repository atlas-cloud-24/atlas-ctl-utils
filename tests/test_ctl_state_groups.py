...
class SelectorGroupResolverTests(unittest.TestCase):
    ...
    def test_resolves_exactly_one_member(self):
        value = run_selectors.resolve_selector_group_member(
            ...
            label="cfg_file_set group state_backend",
        )
    ...
    def test_in_selector_member(self):
        value = run_selectors.resolve_selector_group_member(
            self.GROUP,
            {"execution_context.params.domain": "test"},
            value_field="cfg_file_set_key",
            label="cfg_file_set group state_backend",
        )
    ...
    def test_no_match_is_hard_error(self):
        with self.assertRaisesRegex(RuntimeError, "matched 0"):
            run_selectors.resolve_selector_group_member(
                self.GROUP,
                {"execution_context.params.domain": "identity"},
                value_field="cfg_file_set_key",
                label="cfg_file_set group state_backend",
            )
...
class TargetDomainKeysInventoryTests(unittest.TestCase):
    ...
    def test_generic_domain_resolves_from_the_context(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = self._root(tmp)
            inv = target_catalog.TargetCatalog.action_cfg(
            ...
            self.assertEqual(target["cfg_keys"], {"org": ["main_tag", "tfstate_s3_bucket_name"]})

    def test_generic_domain_unresolved_without_context(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = self._root(tmp)
            inv = target_catalog.TargetCatalog.action_cfg(root, "provision")
            target = inv["targets"]["lz/tfstate_backend"]
    ...
    def test_unknown_domain_is_rejected(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaisesRegex(RuntimeError, "unknown domain"):
                target_catalog.TargetCatalog.action_cfg(
    ...
    def test_cfg_keys_for_an_undeclared_domain_is_rejected(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaisesRegex(RuntimeError, "not in its domains"):
                target_catalog.TargetCatalog.action_cfg(root, "provision", {})
...
class TargetInputParamsTests(unittest.TestCase):
    ...
    def _root(self, tmp: str, target_body: str) -> Path:
        ...
        _write(root, "domains.yaml", "domains:\n  env: {}\n")
        _write(root, "cfg_key_sets.yaml", "cfg_key_sets:\n  k:\n    cfg_keys: [main_tag]\n")
        _write(
            root,
            "param_sets.yaml",
    ...
    def test_param_set_expands_and_instance_subset_passes(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            t = target_catalog.TargetCatalog.action_cfg(root, "provision", {})["targets"]["t"]
    ...
    def test_static_vars_reach_the_run_and_the_target_context(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            action_cfg = target_catalog.TargetCatalog.action_cfg(root, "provision", {})
            self.assertEqual(action_cfg["targets"]["t"]["static_vars"], {"plt_scope": "ecr_repos"})
            runs = target_catalog.ActiveTargetRuns.build(
                {"target_runs": [{"id": "t", "target": "t"}], "meta": {"action": "provision"}},
                action_cfg,
            ...
            self.assertEqual(context["execution_context.target.static_vars.plt_scope"], "ecr_repos")
...
class CfgKeyProjectionTests(unittest.TestCase):
    ...
    DOC = {
        "main_tag": "oxygen",
        "foundation": {"networking": {"vpc": 1}, "dns": 2},
        "plt_a_tfstate_key": "a",
        "plt_b_tfstate_key": "b",
    }
    ...
    def test_exact_glob_and_dotted_path(self):
        self.assertEqual(
            execution_references.project_cfg_keys(self.DOC, ["main_tag"], label="t"),
            {"main_tag": "oxygen"},
        )
        self.assertEqual(
...
class ChildLockGrantTests(unittest.TestCase):
    ...
    def test_grant_is_single_use(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = self._root(tmp)
            grant = state_run_store.mint_child_lock_grant(
                root, child_kind="target", child_key="t/a"
            )
            self.assertTrue(
                state_run_store.consume_child_lock_grant(
