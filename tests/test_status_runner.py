"""Standalone read-only status runner.
...
def _seed_workflow_pointer(root: Path, action: str, key: str, seg: str, *, when: str, status: str = "ok", child_revisions=None):
    ...
    kernel_yaml_io.write_yaml_file(d / "committed.yaml", payload)
    return d / "committed.yaml"
...
class FinalizeStatusArgsTests(unittest.TestCase):
    def setUp(self):
        ...
        # which the engine reaches only through the declared providers
    ...
    def test_local_requires_root(self):
        with self.assertRaises(RuntimeError):
            cli_args.finalize_status_args(self._ns(all=True, ctl_state_local_root=None))
    ...
    def test_targeted_requires_action(self):
        with self.assertRaises(RuntimeError):
            cli_args.finalize_status_args(self._ns(target="env/seed/baseline", action=None))
